...
export default function ExamCodeLoginPage({
  params,
}: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/exam/login", {
        ...
        body: JSON.stringify({ exam_id: examId, code: code.trim() }),
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      ...
      >
        <div className="text-center space-y-1">
          ...
          <p className="text-sm text-slate-500">Nhập mã số thí sinh được cấp cho đề thi này.</p>
        </div>
